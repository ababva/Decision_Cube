...
По умолчанию PostgreSQL запускается на порту 5433 (если на вашем компьютере уже установлен PostgreSQL, он обычно занимает порт 5432). Если хотите изменить порт, откройте файл `env` и измените значение `POSTGRES_PORT`.
...
2. **Запустите приложение:**
   - Нажмите зеленую кнопку Run (▶️) вверху Android Studio
...
1. В приложении зарегистрируйте нового пользователя
2. Выполните бросок кубика несколько раз
3. Проверьте статистику - должны отображаться выполненные упражнения
4. Посмотрите рейтинг - ваш пользователь должен там появиться

Если что-то не работает:
- Проверьте логи backend в консоли, где запущен `./gradlew run`
- Проверьте логи Android приложения в Android Studio (вкладка Logcat)
- Убедитесь, что все три компонента (база данных, backend, приложение) запущены
...
Если нужно остановить базу данных полностью (включая удаление данных):

```bash
docker-compose down -v
```

Но будьте осторожны - это удалит все данные из базы.
